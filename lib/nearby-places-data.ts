...
/**
 * 일자별 일정 Day 카드에 쓸 관광지·맛집 후보를 최대 limit개 뽑는다.
 * 관광지·맛집을 번갈아 섞어(관광지1, 맛집1, 관광지2, ...) 한쪽 카테고리로
 * 치우치지 않게 한다. 큐레이션 후보가 limit보다 적은 도시는 있는 만큼만
 * 돌려준다.
 *
 * 같은 도시에 여러 날 머물 때(dayIndexInStay/totalDaysInStay) 매 Day마다
 * 후보 목록이 겹치지 않도록, 전체 후보를 체류 일수만큼 구간으로 나누고
 * 그중 dayIndexInStay번째 구간을 순환(모듈로)으로 뽑는다. 후보 수가 적어
 * 체류 일수를 못 채우면 처음부터 다시 순환한다.
 */
export function getDayHighlights(
  ...
  const pool = interleaveByCategory(getNearbyPlaces(cityId));
